...
本文指导你创建一个 Telegram 机器人，并获取 BinanceAlpha 所需的两个配置：
...
2. 发送 `/newbot`。
3. 按提示输入：
   - **机器人名称**（display name）：随意，如 `BinanceAlpha Monitor`。
   - **用户名**（username）：必须以 `bot` 结尾，如 `my_binance_alpha_bot`。
...
先完成项目依赖安装，然后在项目根目录发送一条测试消息：

```bash
uv sync --locked
uv run python -c "import asyncio; from telegram_notifier import send_message_async; asyncio.run(send_message_async('✅ BinanceAlpha 测试消息，推送通道正常'))"
```
