...
4. 代币分配与解锁（基础门槛，权重10%）：
   - MC/FDV比例：反映代币解锁压力，比例越高越好, 只需要对MC/FDV小于100%的代币进行分析解锁压力
...
2. 核心优势：每个TOP3项目的1-3个最突出的优势, MC/FDV = 1时，不作为核心优势, 但需要分析解锁压力
...
2. 四大因素加权分析：
   ...
   - 价格稳定性(40%)：详细分析价格波动情况及稳定性趋势
   - 监管合规性(5%)：项目合规状况及团队背景评估
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            if usage:
                                logger.info(f"Token使用 - 输入: {usage.get('prompt_tokens')}, "
                                          f"输出: {usage.get('completion_tokens')}, "
                                          f"总计: {usage.get('total_tokens')}")
                            
                            # 验证内容有效性
                            if content and len(content) > 100:
                                logger.info(f"成功获取AI建议，长度: {len(content)}字符")
                                return content
                            else:
                                logger.warning(f"返回内容过短或为空，长度: {len(content)}字符")
